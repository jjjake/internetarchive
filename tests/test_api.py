...
def test_get_item_with_kwargs():
    with IaRequestsMock(assert_all_requests_are_fired=False) as rsps:
        ...
        assert isinstance(item.session.adapters[f'{PROTOCOL}//'].max_retries, urllib3.Retry)
...
def test_modify_metadata():
    with IaRequestsMock(assert_all_requests_are_fired=False) as rsps:
        ...
        rsps.add(
            ...
            f'{PROTOCOL}//archive.org/metadata/nasa',
            body=(
                '{"success":true,"task_id":423444944,'
...
def test_upload():
    ...
    with IaRequestsMock(assert_all_requests_are_fired=False) as rsps:
        rsps.add(
            responses.PUT, re.compile(r'.*s3.us.archive.org/.*'), adding_headers=expected_s3_headers
        )
        rsps.add_metadata_mock('nasa')
        rsps.add(responses.GET, f'{PROTOCOL}//archive.org/metadata/nasa', body='{}')
        _responses = upload(
            'nasa', NASA_METADATA_PATH, access_key='test_access', secret_key='test_secret'
        )
        for response in _responses:
            req = response.request
            headers = {k.lower(): str(v) for k, v in req.headers.items()}
            scanner_header = '%20'.join(response.headers['x-archive-meta00-scanner'].split('%20')[:4])
...
def test_download(tmpdir):
    ...
    with IaRequestsMock() as rsps:
        rsps.add(
            responses.GET, f'{PROTOCOL}//archive.org/download/nasa/nasa_meta.xml', body='test content'
        )
        rsps.add_metadata_mock('nasa')
...
def test_search_items(session):
    ...
    with IaRequestsMock(assert_all_requests_are_fired=False) as rsps:
        rsps.add(responses.POST, results_url, body=TEST_SCRAPE_RESPONSE, match_querystring=True)
        rsps.add(
            responses.POST,
            count_url,
...
def test_search_items_with_fields(session):
    ...
    with IaRequestsMock() as rsps:
        rsps.add(responses.POST, results_url, match_querystring=True, body=search_response_str)
        rsps.add(
            responses.POST,
            count_url,
            body='{"items":[],"count":0,"total":1}',
        ...
        r = search_items('identifier:nasa', fields=['identifier', 'title'], archive_session=session)
...
def test_page_row_specification(session):
    ...
    with IaRequestsMock(assert_all_requests_are_fired=False) as rsps:
        ...
        r = search_items(
            'identifier:nasa', params={'page': '1', 'rows': '1'}, archive_session=session
        )
        assert r.iter_as_items().search == r
