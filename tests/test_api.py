...
def test_get_files_with_get_item_kwargs(tmpdir):
    ...
    with IaRequestsMock(assert_all_requests_are_fired=False) as rsps:
        ...
        files = get_files(
            'nasa', files='nasa_meta.xml', config={'logging': {'level': 'INFO'}}
        )
        files = list(files)
        assert len(files) == 1
        assert files[0].name == 'nasa_meta.xml'
        ...
        files = get_files(
            'nasa', files='nasa_meta.xml', http_adapter_kwargs={'max_retries': 3}
        )
        files = list(files)
        assert len(files) == 1
        assert files[0].name == 'nasa_meta.xml'
...
def test_search_items_with_fields(session):
    ...
    with IaRequestsMock() as rsps:
        ...
        r = search_items(
            'identifier:nasa', fields=['identifier', 'title'], archive_session=session
        )
...
def test_search_items_fts(session):
    with IaRequestsMock(assert_all_requests_are_fired=False) as rsps:
        ...
        r = search_items(
            'nina simone', full_text_search=True, dsl_fts=True, archive_session=session
        )
        assert r.fts == True
...
def test_page_row_specification(session):
    ...
    with IaRequestsMock(assert_all_requests_are_fired=False) as rsps:
        rsps.add(
            responses.GET, f'{PROTOCOL}//archive.org/advancedsearch.php', body=_search_r
        )
        rsps.add_metadata_mock('nasa')
        rsps.add(
            responses.POST,
