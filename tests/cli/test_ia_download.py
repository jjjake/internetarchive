...
def test_clobber(tmpdir_ch):
    ...
    expected_stderr = (
        f'{prefix} skipping {filepath}, file already exists based on length and date.'
    )
...
def test_checksum(tmpdir_ch):
    ...
    filepath = os.path.join('nasa', 'nasa_meta.xml')
    assert (
        f'{prefix} skipping {filepath}, file already exists based on checksum.'
        == stderr
    )
...
def test_checksum_archive(tmpdir_ch):
    ...
    _stdout, stderr = call_cmd(
        'ia --insecure download --checksum-archive nasa nasa_meta.xml'
    )
    assert files_downloaded('nasa') == {'nasa_meta.xml'}
    prefix = 'nasa:\n'.replace('\n', os.linesep)
    filepath = os.path.join('nasa', 'nasa_meta.xml')
    ...
    filepath = os.path.join('nasa', 'nasa_meta.xml')
    assert (
        f'{prefix} skipping {filepath}, file already exists based on checksum_archive.'
        == stderr
    )
...
def _range_headers(rsps):
    ...
    return [
        c.request.headers['Range'] for c in rsps.calls if 'Range' in c.request.headers
    ]
