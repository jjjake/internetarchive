...
class BaseItem:
    ...
    def __hash__(self) -> int:
        without_excluded_keys = {
            k: v for k, v in self.item_metadata.items() if k not in self.EXCLUDED_ITEM_METADATA_KEYS
        }
        return hash(
            json.dumps(
                without_excluded_keys, sort_keys=True, check_circular=False  # type: ignore[call-arg]
            )
        )
        ...
        for f in files:
            if isinstance(f, dict):
                if f.get('name'):
                    ...
                    f = f['name']
            if (isinstance(f, str) and is_dir(f)) or (isinstance(f, tuple) and is_dir(f[-1])):
...
class Collection(Item):
    ...
    def _do_search(self, name: str, query: str):
        rtn = self.searches.setdefault(name, self.session.search_items(query, fields=['identifier']))
