...
class MetadataPreparedRequest(requests.models.PreparedRequest):
    ...
    def prepare_body(self, metadata, source_metadata, target, priority, append, append_list):
        ...
        if (
            ...
            for key in metadata:
                if key == 'metadata':
                    try:
                        patch = prepare_patch(
                            metadata[key], source_metadata['metadata'], append, append_list
                        )
                    except KeyError:
                        raise ItemLocateError
                elif key.startswith('files'):
                    patch = prepare_files_patch(
                        metadata[key], source_metadata['files'], append, key, append_list
                    )
                else:
                    key = key.split('/')[0]
        ...
        # Write to single target
            ...
            if not target or 'metadata' in target:
                target = 'metadata'
                try:
                    patch = prepare_patch(metadata, source_metadata['metadata'], append, append_list)
                except KeyError:
                    raise ItemLocateError
