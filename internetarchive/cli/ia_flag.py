...
def main(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    ...
    if args.add_flag:
        r = item.add_flag(args.add_flag, args.user)
        j = r.json()
        if j.get("status") == "success":
            print(f"success: added '{args.add_flag}' flag to {args.identifier}")
        else:
            print(f"error: {item.identifier} - {r.text}")

    elif args.delete_flag:
        r = item.delete_flag(args.delete_flag, args.user)
        j = r.json()
        if j.get("status") == "success":
            print(f"success: deleted flags marked '{args.delete_flag}' from {args.identifier}")
        else:
            print(f"error: {item.identifier} - {r.text}")
