    ...
    s = Schema(
        {
            ...
            '<file>': And(
                ...
                And(
                    lambda f: False if f == ['-'] and not args['--remote-name'] else True,
            ...
            for row in spreadsheet:
                for metadata_key in row:
                    if not is_valid_metadata_key(metadata_key):
                        print(
                            f'error: "{metadata_key}" is not a valid metadata key.', file=sys.stderr
                        )
                        sys.exit(1)
                ...
                if not identifier:
                    if not prev_identifier:
                        print('error: no identifier column on spreadsheet.', file=sys.stderr)
                        sys.exit(1)
                ...
                r = _upload_files(item, local_file, upload_kwargs_copy, prev_identifier, session)
                for _r in r:
                    if args['--debug']:
                        break
                    if (not _r.status_code) or (not _r.ok):
