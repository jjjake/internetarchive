...
def test_needs_quote():
    notascii = (
        'ȧƈƈḗƞŧḗḓ ŧḗẋŧ ƒǿř ŧḗşŧīƞɠ, ℛℯα∂α♭ℓℯ ♭ʊ☂ η☺т Ѧ$☾ℐℐ, ¡ooʇ ןnɟǝsn sı uʍop-ǝpısdn'
    )
...
def test_IdentifierListAsItems_len(session):
    assert (
        len(internetarchive.utils.IdentifierListAsItems(['foo', 'bar'], session)) == 2
    )
...
def test_is_windows():
    ...
    with patch('platform.system', return_value='Linux'), patch('sys.platform', 'linux'):
