...
class Search:
    ...
    def _scrape(self):
        ...
        while True:
            r = self.session.post(
                self.scrape_url, params=self.params, auth=self.auth, **self.request_kwargs
            )
            j = r.json()
            if j.get('error'):
                yield j
    ...
    def _full_text_search(self):
        ...
        while True:
            r = self.session.post(
                self.fts_url, params=self.params, json=d, auth=self.auth, **self.request_kwargs
            )
            j = r.json()
    ...
    def num_found(self):
        if not self._num_found:
            if not self.fts:
                ...
                r = self.session.get(
                    self.fts_url, params=self.params, auth=self.auth, **self.request_kwargs
                )
                j = r.json()
