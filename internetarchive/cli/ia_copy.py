...
def main(
    ...
    try:
        ...
        print('error: The source and destination files cannot be the same!', file=sys.stderr)
        sys.exit(1)
    ...
    if not args['--replace-metadata']:
        args['--metadata'] = merge_dictionaries(SRC_ITEM.metadata, args['--metadata'])  # type: ignore
