...
class ArchiveSession(requests.sessions.Session):
    ...
    def submit_task(
        ...
        return catalog.Catalog(self, request_kwargs).submit_task(
            identifier, cmd, comment=comment, priority=priority, data=data, headers=headers
        )
    ...
    def get_tasks_summary(
        self, identifier: str = '', params: dict | None = None, request_kwargs: Mapping | None = None
    ) -> dict:
    ...
    def get_tasks(
        ...
        if 'catalog' not in params:
            params['catalog'] = 1
        return set(
            catalog.Catalog(self, request_kwargs).get_tasks(identifier=identifier, params=params)
        )
