        ...
        try:
            return hash(
                json.dumps(
                    without_excluded_keys,
                    sort_keys=True,
                    check_circular=False,  # type: ignore[call-arg]
                )
            )
        except AttributeError:  # ujson doesn't support check_circular
            return hash(json.dumps(without_excluded_keys, sort_keys=True))
